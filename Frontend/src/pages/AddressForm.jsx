...
const AddressForm = () => {
  ...
  const { addresses = [], selectedId = null } = useSelector(
    (state) => state.address || {},
  );
  ...
  const getAddresses = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_URL}/api/address/getAllAddresses`, {
        headers: {
      ...
      if (error.response?.status === 401) {
        ...
        toast.error(
          `Failed to fetch addresses - ${error.response?.data?.message || error.message}`,
        );
      }
    } finally {
      setLoading(false);
  ...
  const handleAddAddress = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${API_URL}/api/address/add`, formData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
        },
      });
      ...
      toast.error(
        `Failed to add address - ${error.response?.data?.message || error.message}`,
      );
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      if (data.success) {
        const options = {
          ...
          prefill: {
            name:
              addresses.find((addr) => addr._id === selectedId)?.fullName || "",
            contact:
              addresses.find((addr) => addr._id === selectedId)?.phone || "",
          },
          theme: {
            color: "#0f4d0e",
          },
  ...
  const verifyPayment = async (paymentData) => {
    try {
      ...
      toast.error(
        `Payment verification error - ${error.response?.data?.message || error.message}`,
      );
    }
  };

  return (
    <div className="container mx-auto px-4 py-6 mt-16 md:mt-16 md:my-10">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between px-2 mb-8 gap-4">
            ...
            >
              ...
              <DialogContent className="w-[90%] max-w-[500px] rounded-[2rem] p-6">
                <form
                  onSubmit={isEditing ? handleEditAddress : handleAddAddress}
                >
                  <DialogHeader>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <select
                          ...
                          onChange={handleChange}
                          className="flex h-11 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-primary outline-none text-muted-foreground"
                        >
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          disabled
                          value="India"
                          className="rounded-xl bg-muted h-11"
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <DialogFooter className="gap-2 mt-4 flex-col-reverse sm:flex-row">
                    ...
                    <Button
                      type="submit"
                      disabled={loading}
                      className="rounded-xl gradient-primary px-8 w-full sm:w-auto"
                    >
                      {loading ? (
            ...
            {loading ? (
              <div className="space-y-4">
                {[1, 2].map((item) => (
                  <div
                    key={item}
                    className="p-6 border rounded-[2rem] bg-muted/30 animate-pulse h-28"
                  />
                ))}
              </div>
            ) : (
                    ...
                    >
                      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                        <div className="flex gap-4 items-start w-full sm:w-auto">
                          <div
                            className={`p-3 rounded-2xl h-12 w-12 flex items-center justify-center ${selectedId === addr._id ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}
                          >
                            <MapPin size={20} />
                          </div>
                        ...
                        <div className="flex items-center gap-2 self-end sm:self-center w-full sm:w-auto justify-between sm:justify-start border-t border-border/50 pt-3 sm:pt-0 mt-2 sm:mt-0">
                          ...
                          {selectedId === addr._id && (
                            <CheckCircle2 className="text-primary h-5 w-5 ml-3" />
                          )}
                        </div>
                      </div>
                    </motion.div>
          ...
          <aside className="w-full lg:w-96 lg:sticky lg:top-32">
            ...
            >
              ...
              <Button
                ...
                className="w-full mt-8 h-14 rounded-2xl text-lg font-bold group shadow-lg shadow-primary/20"
              >
                {addingToCart ? (
